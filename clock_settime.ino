...
#include <iarduino_RTC.h>
  iarduino_RTC time(RTC_DS1302, 2, 3, 4); // подключаем RTC модуль на базе чипа DS1302, указывая выводы Arduino подключённые к выводам модуля RST, CLK, DAT
...
void setup() {
    ...
    Serial.begin(9600);
    time.begin();
    //time.settime(0,51,21,27,10,18,2);  // 0  сек, 51 мин, 21 час, 27, октября, 2018 года, вторник
    time.settime(-1,-1,-1,-1,-1,-1,3);
}
void loop(){
